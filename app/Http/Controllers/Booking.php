...
////
        ...
        $busTypes = BusType::inRandomOrder()
            ->limit(10)
            ->get();
        ...
        $destination = Destination::where('id', $data['destination_to'])->select('location')->first();


        //find the type of trip
        $tripType = TripType::where('id', $data['trip_type'])->select('type', 'id')->first();

        !$tripType ? abort(404) : $request->session()->put('tripType', $data['trip_type']);

        $service = Service::where('id', $data['service_id'])->select('name')->first();
    ...
    public function seatSelector($schedule_id, $tripType)
    {
        ...
        $fetchSeats = SeatTracker::where('schedule_id', $schedule_id)
            ->select('seat_position', 'id', 'booked_status')
            ->get();
    ...
    public function selectorTracker(Request $request)
    {
        ...
        $seat = SeatTracker::where('id', $data['seat_id'])->first();
        ...
        if ($seat->booked_status != 2) {
            ...
            if ((int)$request->trip_type == 2) {
                ...
                if ($returnTripSchedule) {
                    $findReturnScheduleTripSeat = SeatTracker::where('schedule_id', $returnTripSchedule->id)
    ...
    public function deselectSeat(Request $request)
    {
        ...
        if (!isset($request->onsite_customer_id)) $seat = SeatTracker::where('id', $data['seat_id'])->where('user_id', $request->user_id)->first();
        ...
        if ($passengerCount != $passengerOptionCount) {
            ...
            foreach ($passenger_options as $passenger_option) {
                if (strtolower($passenger_option) == 'adult') {
                    $adult[] = $passenger_option;
                } elseif (strtolower($passenger_option) == 'children') {
                    $children[] = $passenger_option;
                }
...
//                        'tripType' => 'required'
        ...
        $tripSchedule = Schedule::where('id', $attr['schedule_id'])
