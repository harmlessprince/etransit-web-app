...
class ScheduleImport implements ToModel, WithStartRow, WithBatchInserts, WithChunkReading
{
    ...
    public function model(array $row)
    {
        ...
        $bus_registration = Bus::where("bus_registration", "=", $row[2])->select('id', 'seater', 'tenant_id')->first();
        $terminal_id = Terminal::withoutGlobalScopes()->where("terminal_name", "=", $row[0])->select('id')->first();
        $service_id = Service::where("name", "=", $row[1])->select('id')->first();
        $pickup_id = Destination::where("location", "=", $row[3])->select('id')->first();
        $destination_id = Destination::where("location", "=", $row[4])->select('id')->first();
